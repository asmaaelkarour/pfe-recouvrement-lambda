...
client_agg = client_agg.withColumn(
    "taux_recouvrement",
    F.round(
        F.col("montant_recouvre") * 100 / F.nullif(F.col("montant_du"), F.lit(0)),
        2
...
# ─────────────────────────────────────────────
monthly_agg = silver_df.groupBy("year", "month", "categorie_client").agg(
    F.count("record_id").alias("nb_dossiers"),
    F.sum("montant").cast(DecimalType(18, 2)).alias("total_montant_du"),
    F.sum(
        F.when(F.col("statut") == "SOLDE", F.col("montant")).otherwise(F.lit(0))
    ).cast(DecimalType(18, 2)).alias("total_montant_recouvre"),
)

monthly_agg = monthly_agg.withColumn(
    "taux_recouvrement_mensuel",
    F.round(
        F.col("total_montant_recouvre") * 100 / F.nullif(F.col("total_montant_du"), F.lit(0)),
        2
    ).cast(DecimalType(5, 2))
)
...
print(f"[Silver→Gold] ETL completed. Client aggregations: {client_agg.count()}")
